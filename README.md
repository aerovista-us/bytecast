# ByteCast Workspace

This is the root entrypoint for the ByteCast multi-project workspace.

## Start here
1. Open `index.html` in a browser, or run a local server:
   - `python -m http.server 8080`
   - then open `http://localhost:8080`
2. Use the Smart Workflow panel:
   - complete `ByteCast (listen -> slide -> interact) -> Training Hub -> Seed Builder real work`
   - each completed cycle unlocks the next Seed Builder ability level

## Primary apps
- ByteCast App: `seed_bytecast.html`
- Training Hub: `training_hub/index.html`
- Seed Builder Studio: `seed_builder_studio/index.html`

## Docs and governance
- Docs index: `docs/README.md`
- App shells and ownership: `docs/APP_SHELLS.md`
- CANON map: `.CODEX/bytecast_canon_map_2026-02-08.md`
- Migration plan: `.CODEX/episode_content_naming_and_2app_migration_plan_2026-02-08.md`
- Episode registry: `.CODEX/episode_registry.json`
- End of session report: `.CODEX/end_of_session_report_2026-02-08.md`

## Compatibility + archive
- Legacy wrappers (until review cycle): `ep1/`, `ep3/`
- Retired paths archive: `_archive/phase4_retired_2026-02-08/`
