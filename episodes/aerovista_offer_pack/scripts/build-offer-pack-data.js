 ...
 * Run from repo root: node episodes/aerovista_offer_pack/scripts/build-offer-pack-data.js
...
function parseOfferCardMd(content, divisionId, offerKey) {
  ...
  const titleMatch = content.match(/^#\s+(.+?)(?:\s+â€”\s+Offer Card)?$/m);
  const title = titleMatch ? titleMatch[1].replace(/^[^â€”]+â€”\s*/, '').trim() : offerKey;
  return {
...
const OFFER_NAME_TO_KEY = {
  ...
  echoverse: {
    'Brand Sonic Logo (5â€“8 sec)': 'sonic-logo',
  ...
  summit: {
    'SOP in a Box': 'sop-in-a-box',
    'Micro-Training Module (10â€“15 min)': 'micro-training',
...
function parseOnePager() {
  ...
  return {
    ...
    divisions,
    popularBundles,
    standardTerms: terms ? terms[1].trim() : '50% to schedule Â· two revision rounds Â· clear client-provided asset list.',
  };
}
