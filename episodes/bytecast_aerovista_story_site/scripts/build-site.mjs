...
async function main() {
  ...
  const fileNames = (await fs.readdir(episodesDir))
  ...
  for (const doc of ordered) {
    ...
    const outputPath = path.join(episodesDir, `${doc.slug}.html`);
  ...
  const generatedSlugsHtml = new Set(ordered.map((doc) => path.basename(doc.outputName)));
  ...
  console.log(
    `Built ${ordered.length} episode pages under episodes/ + index.html + site-map.html${ttsConfig.enabled ? " (TTS enabled)" : ""}.`,
  );
}
...
function parseDocument(fileName, rawText) {
  ...
  const rawNarration = buildNarrationText({ ...baseMeta, structuredScript }, sections);
  const narrationText = applyDocumentaryPacingProse(rawNarration);
  ...
  return {
    ...baseMeta,
    fileName,
    outputName: `episodes/${baseMeta.slug}.html`,
    preview,
    sections,
    theme,
    narrationText,
...
/**
 * Aligns with Narrator’s “documentary” intent: extra breath at punctuation for
 * OpenAI TTS and browser read-aloud (newlines). Set BYTECAST_DOC_PACING=0 to skip.
 */
function applyDocumentaryPacingProse(text) {
  if (process.env.BYTECAST_DOC_PACING === "0") {
    return text;
  }
  ...
  // Longer hold on ellipsis (and avoid eating "...." as multiple passes)
  s = s.replace(/\.{3,}/g, (m) => `${m.trim()}\n\n`);
  // Exclamation / question
  s = s.replace(/[!?]+\s+/g, (m) => `${m.trim()}\n\n`);
  // Period ends a sentence (avoid decimals like 3.14 — require letter/quote before dot)
  s = s.replace(/(?<=[A-Za-z)"'])\.(?=\s+)/g, ".\n\n");
  // Comma: light breath (documentary, easy listening)
  s = s.replace(/,\s+/g, ",\n");
  s = s.replace(/\n{3,}/g, "\n\n");
  return s.trim();
}
...
function getTtsConfig() {
  return {
    ...
    speed: Number(process.env.BYTECAST_TTS_SPEED || "0.88"),
    instructions:
      process.env.BYTECAST_TTS_INSTRUCTIONS ||
      "Warm cinematic documentary narration. Slow, deliberate pacing with generous breathing room between phrases and ideas. Crystal-clear articulation; grounded and authoritative but approachable. Premium broadcast storytelling tone optimized for easy listening.",
  };
}
...
function renderDocumentPage(doc) {
  ...
  const audioSources = doc.audio.sources.map((source) => `../${source}`).join("|");
...
function episodeRelativeHref(href) {
  const value = String(href || "");
  return value.startsWith("episodes/") ? value.slice("episodes/".length) : `../${value}`;
}
...
function renderSiteMapPage(docs, supplementaryHtmlFiles) {
  ...
  const generatedLinks = docs
    .map((doc) => {
      ...
      return `<li class="site-map-row"><a href="${escapeHtml(doc.outputName)}">${escapeHtml(doc.title)}</a><span class="site-map-meta">${escapeHtml(
        badge,
