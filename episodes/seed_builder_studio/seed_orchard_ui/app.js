(() => {
    ...
    const slides = Array.isArray(artifact?.fruits)
      ? artifact.fruits.map((fruit, idx) => {
        ...
        const bullets = [
          ...
          [effort, signal].filter(Boolean).join(" â€¢ ") || "Keep it small. Keep it shippable.",
        ];
        return {
          id: `s${idx + 1}`,
          title,
      ...
      : [
        {
          ...
          bullets: ["Plant â†’ Harvest â†’ Export", "Make it repeatable", "Prove it with an artifact"],
        },
      ];
    ...
    return {
        ...
        quiz: {
          ...
          questions: [
            ...
            },
            {
              ...
              answer_index: 1,
              explain: "Seed â†’ artifact â†’ proof â†’ iteration.",
            },
          ],
        },
      },
      aerovista_about: {
        ...
        divisions: [
          ...
          { name: "Seed Orchard", bread_and_butter: "Seed-to-fruit workflow: validate â†’ export â†’ badge." },
        ],
      },
    };
  }
            ...
            steps: [
              {
                ...
                label: "EP-001 Gates",
                lane: "bytecast",
                href: "episodes/welcome_to_bytecast/index.html",
                complete_when: { type: "steps_all", ids: ["ep001_listen", "ep001_slide", "ep001_engage"] },
              },
              {
                id: "ep002_gates",
                label: "EP-002 Gates",
                lane: "bytecast",
                href: "episodes/aerovista_7_division_overview/index.html",
                depends_on: ["ep001_gates"],
                unlock_requires: [{ stepId: "ep001_engage", meta_fields: ["engageQuizPassed", "engageQuizScore"] }],
                complete_when: { type: "steps_all", ids: ["ep002_listen", "ep002_slide", "ep002_engage"] },
              },
              {
                id: "ep003_gates",
                label: "EP-003 Gates",
                lane: "bytecast",
                href: "episodes/the_main_doors/index.html",
                depends_on: ["ep002_gates"],
                unlock_requires: [{ stepId: "ep002_engage", meta_fields: ["engageQuizPassed", "engageQuizScore"] }],
                complete_when: { type: "steps_all", ids: ["ep003_listen", "ep003_slide", "ep003_engage"] },
              },
              {
                id: "ep004_gates",
                label: "EP-004 Gates",
                lane: "bytecast",
                href: "episodes/current_truth_basics/index.html",
                depends_on: ["ep003_gates"],
                unlock_requires: [{ stepId: "ep003_engage", meta_fields: ["engageQuizPassed", "engageQuizScore"] }],
                complete_when: { type: "steps_all", ids: ["ep004_listen", "ep004_slide", "ep004_engage"] },
              },
              {
                id: "tr001a_day1_foundations",
                label: "TR-001A Day 1 Check",
                lane: "training",
                href: "episodes/training_missions/tr_001a_day1_foundations/index.html",
                depends_on: ["ep004_gates"],
                unlock_requires: [{ stepId: "ep004_engage", meta_fields: ["engageQuizPassed", "engageQuizScore"] }],
              },
              {
                id: "seed_export_v1",
                label: "Seed Export",
                lane: "seed",
                href: "episodes/seed_builder_studio/seed_orchard_ui/index.html",
                depends_on: ["tr001a_day1_foundations"],
                unlock_requires: [
                  {
              ...
              },
              {
                ...
                lane: "seed",
                href: "episodes/seed_builder_studio/seed_orchard_ui/index.html",
                depends_on: ["seed_export_v1"],
                unlock_requires: [{ stepId: "seed_export_v1", meta_fields: ["artifactName", "artifactHash"] }],
                complete_when: { type: "step_done", id: "seed_publish_v1" },
              },
              {
                id: "badge_p1_golden_path_v1",
                label: "Badge",
                lane: "badge",
                href: "seed_bytecast.html",
                depends_on: ["seed_publish_v1"],
                complete_when: { type: "badge_has", badge_id: "p1_golden_path_v1" },
              },
            ],
            badges: [
              {
                ...
                label: "P1: Golden Path",
                requires: ["ep001_gates", "ep002_gates", "ep003_gates", "ep004_gates", "tr001a_day1_foundations", "seed_export_v1", "seed_publish_v1"],
                minProof: {
  ...
  async function refreshExportUI() {
    ...
    const trainingOk = (Loop && journey && wf2)
      ? (
          Loop.isStepDone(wf2, "tr001a_day1_foundations") ||
          Loop.isStepDone(wf2, "tr001_golden_path") ||
          (journey.steps || []).some((s) => s && (s.id === "tr001a_day1_foundations" || s.id === "tr001_golden_path") && Loop.isStepComplete(journey, s, wf2))
        )
      : trainingOkLegacy;
    ...
    const prereqStatus = (() => {
      ...
      return { ok: Loop.isStepUnlocked(journey, seedStep, wf2) };
    })();
  ...
  form.addEventListener("submit", (event) => {
    ...
    lastArtifact = {
      ...
      tool: "episodes/seed_builder_studio/seed_orchard_ui",
      createdAt: new Date().toISOString(),
      seed: {
        name: data.seedName,
        goal: data.goal,
